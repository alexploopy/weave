...
_WEAVE_ROOT = Path(__file__).resolve().parent / "weave"
...
class WeaveImportBoundaryTests(unittest.TestCase):
    def test_weave_does_not_import_private_transcript(self):
        offenders: list[str] = []
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if alias.name == "transcript" or alias.name.startswith("transcript."):
                            offenders.append(f"{path}: import {alias.name}")
                elif isinstance(node, ast.ImportFrom) and node.module:
                    if node.module == "transcript" or node.module.startswith("transcript."):
